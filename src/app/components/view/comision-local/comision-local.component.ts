...
@Component({
  ...
  templateUrl: './comision-local.component.html',
  styleUrls: ['./comision-local.component.css']
})
export class ComisionLocalComponent implements OnInit {
  ...
  exportAsXLSX(): void {
    this.flagbtn = false;
    var excel = [];
    this.lista_comision_reporte = null;
    this.comisionService.getAllComisionVehReporteServicios(this.user.codigo, 2, 0, this.user.id_ciclo, this.user.id_etapa).subscribe(res => {
  ...
  getAllComisionVehiculo() {
    ...
    this.comisionService.getAllComisionVehServicios(this.user.codigo, 2, 0, this.user.id_ciclo, this.user.id_etapa).subscribe(res => {
  ...
  changeMovilidad() {
    this.destino.tarifa = 0;
    this.destino.id_vehiculo = 0;
    this.destino.id_tipo_carretera = 0;
    this.destino.kilometro = 0;
  }
}
