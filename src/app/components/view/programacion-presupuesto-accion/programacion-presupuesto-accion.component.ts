...
@Component({
  selector: 'app-programacion-presupuesto-accion',
  templateUrl: './programacion-presupuesto-accion.component.html',
  styleUrls: ['./programacion-presupuesto-accion.component.css']
})
export class ProgramacionPresupuestoAccionComponent implements OnInit {
  lista_ciclo: Object;
  lista_ugp: Object;
  ...
  @ViewChild("closeModalMarco", {static: false})
  closeModalMarco: ElementRef;
  ...
  getAllTope() {
    ...
    this.topePresupuestal.getAllMarcoUGP(this.id_ugp, this.user.codigo, 0).subscribe(res => {
      ...
      this.total_generica_marco = this.lista_tope_ugp[0].generica_total;
      ...
      this.total_ugp_ejecutado = this.total_bienes_ejecutado + this.comision_ejecutado + this.rutas_ejecutado;
  ...
  getAllProducto() {
    ...
    //el tope se actualiza al cambiar producto q raro?
    this.getAllTope();
  ...
  getAllPFAcciones() {
    ...
    this.pfaService.getAllPFAcciones3(this.id_accion, this.user.anio, this.user.codigo, 0).subscribe(res => {
      this.pfa = res;
      this.cantidad_lista_pfa = res["length"];
    });
  }
  ...
  getAllBienesPPA() {
    this.x_progreso_bienes = false;
    this.bienesService.getAllBienesServiciosPPA(this.anio, this.user.codigo, this.id_accion, 0).subscribe(
      res => {
        this.x_progreso_bienes = true;
  ...
  sumarTotal(index) {
    var suma = 0;
    var monto = 0;
    for (let k = 0; k < this.bienes_ppa[index].lista_periodo.length; k++) {
      suma = suma + this.bienes_ppa[index].lista_periodo[k].cantidad;
      this.bienes_ppa[index].lista_periodo[k].monto = this.bienes_ppa[index].lista_periodo[k].cantidad * this.bienes_ppa[index].lista_periodo[k].precio_bienes;
      monto = monto + this.bienes_ppa[index].lista_periodo[k].cantidad * this.bienes_ppa[index].lista_periodo[k].precio_bienes;
    }
  ...
  getAllBienesPPAcciones() {
    ...
    this.ppaService.getAllBienesPPAcciones(this.id_accion, this.user.anio, this.user.codigo, 0).subscribe(
      res => {
        // console.log("res",res);
        this.bienes_ppa = res;
        //console.log(res);
  ...
  guardarPPAccion() {
    this.x_progreso = false;
    this.bienes_ppa.id_ciclo = this.user.id_ciclo;
    this.bienes_ppa.id_etapa = this.user.id_etapa;
    this.bienes_ppa.codigo_usuario = this.user.codigo;
    this.ppaService.guardarPPAcciones(this.bienes_ppa).subscribe(res => {
  ...
  getAllBienesCanalizados() {
    this.closeModalMarco.nativeElement.click();
    setTimeout(() => {
        this.route.navigate(["/reporte-programacion-bienes"]), 2000
      }
    )
  }
