...
export class RutasComponent implements OnInit {
  ...
  exportAsXLSX(): void {
    this.flagbtn = false;
    var excel = [];
    this.rutaSrv.listarRutasReporte(this.user.codigo, 0, this.user.id_ciclo, this.user.id_etapa).subscribe(res => {
      this.lista_rutas_reporte = res;

      this.lista_rutas_reporte.forEach(row => {
        let data = {
      ...
      this.flagbtn = true;
      this.excelService.exportAsExcelFile(excel, 'rutas');
    });
  }
  ...
  getAllRutas(){
    this.lista_rutas = null;
    this.rutaSrv.listarRutas(this.user.codigo,0, this.user.id_ciclo, this.user.id_etapa).subscribe(res => {
      this.lista_rutas = res;
      //console.log(res);      this.total_comision = 0;
  ...
  getElemento(elemento){
    this.ruta = {
      ...
      adicional: (elemento!=null) ? elemento.adicional : 0,
    };
